...
const MobileList: React.FunctionComponent<IMobileListProps> = props => {
    ...
    const renderMemberList = () => {
        ...
        if (userRequests?.length > 0) {
            let items: Array<any> = userRequests.map((userRequest: IProjectMemberOverview, index: number) => {
                return {
                                ...
                                <Flex space="between">
                                    <Flex >
                                        <Flex vAlign="center" gap="gap.small">
                                            {userRequest.isBillable && <Status color={Constants.billableStatusColor} title={localize("billable")} />}
                                            {!userRequest.isBillable && <Status color={Constants.nonBillableStatusColor} title={localize("nonBillable")} />}
    ...
    const renderTaskList = () => {
        ...
        if (tasks?.length > 0) {
            let items: Array<any> = tasks.map((task: IProjectTaskOverview, index: number) => {
                return {
                    key: `task-${index}`,
                    content:
                        <div>
