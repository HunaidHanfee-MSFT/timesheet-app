...
interface IRequestReviewState {
    submittedRequests: Array<ISubmittedRequest>
    userTimesheet: Array<IUserTimesheet>,
    isLoading: boolean,
...
class RequestReview extends React.Component<IRequestReviewProps, IRequestReviewState> {
    ...
    getUserTimesheetDetailsAsync = async (selectedWeek: number, userId: string) => {
        ...
        if (response.status === StatusCodes.OK && response.data) {
            let userTimesheets: Array<IUserTimesheet> = response.data;
    ...
    */
     ...
     */
    getProjectTotalHours = (timesheetDetails: Array<ITimesheetDetails>) => {
        let hours = 0;
    ...
    renderTimesheetRequestsList = () => {
        ...
        if (submittedRequests?.length > 0) {
            let items: Array<any> = submittedRequests.map((submittedRequest: ISubmittedRequest, index: number) => {
                return {
