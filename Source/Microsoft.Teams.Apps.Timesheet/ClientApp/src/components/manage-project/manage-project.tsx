...
class ManageProject extends React.Component<IManageProjectProps, IManageProjectState> {
     ...
     */
    getExistingUsers = () => {
        let users: Array<IUserDropdownItem> = this.state.members.map((member: IProjectMemberOverview) => {
    ...
    getMembersOverviewAsync = async (projectId: string, month: number) => {
        ...
        if (response.status === 200 && response.data) {
            let projectMembersOverview: Array<IProjectMemberOverview> = response.data;
     ...
     */
    mapMembersWithDisplayName = (projectMembers: Array<IProjectMemberOverview>, userProfiles: Array<IUser>) => {
    ...
    initializeMonthYearDropdown = () => {
        ...
        let dropdownMonthYear: Array<string> = [];
        filterMonths.map((value: string) => {
            dropdownMonthYear.push(`${value} ${date.getFullYear()}`);
        });
     ...
     */
    onRemoveSelectedItems = (selectedItems: Array<any>, modelType: ModelType) => {
        if (selectedItems) {
            switch (modelType) {
                case ModelType.Task:
     ...
     */
    onRemoveTasks = async (selectedTasks: Array<IProjectTaskOverview>) => {
     ...
     */
    onRemoveMembers = async (selectedMembers: Array<IProjectMemberOverview>) => {
     ...
     */
    AddMembersAsync = async (selectedUsers: Array<IProjectMember>) => {
    ...
    onTaskAdded = async (tasks: ITask[]) => {
        if (tasks.length > 0) {
            let currentMonth = new Date().getMonth();
            let tasksOverview: Array<IProjectTaskOverview> = [];
        ...
        if (!this.params.isMobileView) {
                                ...
                                [
                                    {
                                        ...
                                        content: <ProjectDetails projectDetail={this.state.projectDetails} isMobile={false} />
                                    },
                                    {
            ...
            return (
                <div>
                    ...
                    <Flex vAlign="center">
                        <ProjectDetails projectDetail={this.state.projectDetails} isMobile={true} />
